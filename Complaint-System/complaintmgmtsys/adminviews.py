...
def ADD_STATE(request):
    if request.method == "POST":
        ...
        state.save()
        messages.success(request,'Department has been added succeesfully!!!')
...
def LODGEDCOMPLAINTSREMARK(request):
    if request.method == 'POST':
        ...
        ComplaintRemark.objects.create(
            comp_id_id=lodged_complaint,
            remark=remark_text,
            status=status,
            remarkdate=timezone.now()
        )
        
        # User's email and complaint number
        user_email = lodged_complaint.userregid.admin.email
        complaint_number = lodged_complaint.complaint_number

        # Send emails for specific statuses
        if status == "Inprocess":
            # Email body for "Inprocess"
            email_body = f"""
            Dear {lodged_complaint.userregid.admin.first_name},

            We hope this message finds you well.

            We are writing to inform you that your complaint is currently being processed. Below are the updated details:

            - Complaint Number: {complaint_number}
            - Current Status: {status}
            - Remark: {remark_text}

            Our team is actively working on resolving your concern. We will notify you once the status is updated further.
            ...
            try:
                send_mail(
                    subject="Complaint Status has been changed",
                    message=email_body,
                    from_email=settings.DEFAULT_FROM_EMAIL,
                    recipient_list=[user_email],
                    fail_silently=False,
                )
                messages.success(request, "Status updated to 'Inprocess', and notification email sent to the user.")
            except Exception as e:
                messages.error(request, f"Failed to send email: {str(e)}")

        elif status == "Closed":
            # Email body for "Closed"
            email_body = f"""
            Dear {lodged_complaint.userregid.admin.first_name},

            We hope this message finds you well.

            This is to inform you that your complaint has been resolved and marked as 'Closed.' Below are the final details:
            ...
            try:
                send_mail(
                    subject="Complaint has successfully resolved",
                    message=email_body,
                    from_email=settings.DEFAULT_FROM_EMAIL,
                    recipient_list=[user_email],
                    fail_silently=False,
                )
                messages.success(request, "Complaint closed, and notification email sent to the user.")
            except Exception as e:
                messages.error(request, f"Failed to send email: {str(e)}")
        else:
